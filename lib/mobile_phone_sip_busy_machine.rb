...
class MobilePhoneSipBusyMachine < SipBusyMachine
  ...
  def initialize(params = {})
    super( params.merge(
               { :address => Settings.get('sip_listen_address'),
                 :port => Settings.get('sip_listen_port')
               })
    )
  end
  ...
  def valid_sip_server_address?(params)
    super(params)
    valid_sip_servers = Settings.get('sip_servers').split(',').map { |n| n.strip }
  ...
  def valid_phone_numbers?(params)
    ...
    if params[:from] =~ /\A[0-9]+\Z/ and params[:to] =~ /\A[0-9]+\Z/
      valid_to_numbers = Settings.get('verification_numbers').split(',').map { |n| n.strip }
